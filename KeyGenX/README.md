...
https://github.com/khulnasoft/Docker-OSX/tree/master/KeyGenX/blob/master/CREDITS.md
...
```bash
git clone https://github.com/khulnasoft/Docker-OSX/tree/master/KeyGenX
cd KeyGenX
...
```bash
CUSTOM_PLIST=https://raw.githubusercontent.com/khulnasoft/Docker-OSX/tree/master/KeyGenX/master/config-nopicker-custom.plist
...
[[ "${GENERATE_UNIQUE}" == true ]] && { \
    ./Docker-OSX/scripts/generate-unique-machine-values.sh \
...
; [[ "${GENERATE_SPECIFIC}" == true ]] && { \
        ...
        ; ./Docker-OSX/scripts/generate-specific-bootdisk.sh \
        --master-plist-url="${MASTER_PLIST_URL}" \
        --model "${DEVICE_MODEL}" \
        --serial "${SERIAL}" \
...
patch -F 10 -p1 --verbose configs/config-custom.plist v12...v13.patch
...
Delete lines 641 - 675 in both `configs/config-custom.plist` and `config-nopicker-custom.plist` which is `_cpu_topology_sort`.
...
patch -F 10 -p1 --verbose configs/config-custom.plist v11...v12.patch
```
